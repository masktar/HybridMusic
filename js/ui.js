...
function renderFavoritesPage() {
    ...
    <div class="artist-header" style="background:linear-gradient(to right, #ff5500, #2b1600);">
        <div class="artist-info-container">
            <div style="width:160px; height:160px;margin-bottom:15px; background:white; display:flex; align-items:center; justify-content:center; border-radius:8px; box-shadow:0 10px 30px rgba(0,0,0,0.3);">
...
function renderSongDetailPage(s, ad) {
    ...
    <div class="song-detail-page" style="${bg} width:100%; min-height:100%; padding:40px; box-sizing:border-box; display:flex; flex-direction:column; align-items:center; animation:fadeIn 0.3s; border-radius:20px;">
        <div style="width:100%; display:flex; justify-content:space-between; margin-bottom:20px;">
            ...
            <div style="text-transform:uppercase; font-size:18px; letter-spacing:1px; margin-top:10px; color:#fff; opacity:0.8;">ĐANG PHÁT</div>
            <div style="width:20px;"></div>
        </div>
        ${v}
        <div style="text-align:center; margin-bottom:30px;">
            <div style="display:flex; align-items:center; justify-content:center; gap:15px;">
                <h1 style="font-size:26px; margin:0; color:#fff;">${s.name}</h1>
                <i class="${hc}" style="font-size:24px; flex-shrink: 0; cursor: pointer; transform: translateY(3px);" onclick="toggleHeart('${escapeName(s.name)}', event)"></i>
            </div>
